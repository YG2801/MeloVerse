...
function renderIntitials() {
  ...
  document.querySelector(".popular-songs-cards ul").innerHTML =
    htmlStringForMainSectionSongs;

  shuffleArray(songArray);
  const htmlStringForSideSectionSongs = getHtmlForSideSectionSongs(songArray);
  document.querySelector(".side-bar-song-list ul").innerHTML =
    htmlStringForSideSectionSongs;

  const htmlStringForArtists = getHtmlForArtists(artistArray);
  document.querySelector(".popular-artists-cards ul").innerHTML =
    htmlStringForArtists;
...
function setInitialMusic() {
  ...
  Array.from(sideSongArr)[0].parentElement.style.background =
    "rgb(105,105,170,0.2)";
}
setInitialMusic();
...
function renderSongInfo(songId) {
  ...
  document.querySelector(".player-section .song-title").textContent =
    songDetails[0].title;
  document.querySelector(".player-section .song-artist").textContent =
    songDetails[0].artist;
...
const secondaryProgressBarEl = document.getElementById(
  "secondary-progress-bar"
);
const secondaryProgressThumbEl = document.getElementById(
  "secondary-progress-thumb"
);
...
const secondaryVolumeThumbEl = document.getElementById(
  "secondary-volume-thumb"
);
...
document
  .getElementById("pop-song-scroll-right")
  .addEventListener("click", () => {
    document.querySelector(".popular-songs-cards ul").scrollLeft += 300;
  });
document
  .getElementById("pop-song-scroll-left")
  .addEventListener("click", () => {
    document.querySelector(".popular-songs-cards ul").scrollLeft -= 300;
  });
...
document
  .getElementById("pop-artist-scroll-right")
  .addEventListener("click", () => {
    document.querySelector(".popular-artists-cards ul").scrollLeft += 300;
  });
document
  .getElementById("pop-artist-scroll-left")
  .addEventListener("click", () => {
    document.querySelector(".popular-artists-cards ul").scrollLeft -= 300;
  });

const searchResultContainerEl = document.getElementById(
  ...
  if (inputVal == "") {
    ...
    Array.from(
      document.querySelectorAll(
        ".search .search-results-container .search-result-card .song-title"
      )
    ).forEach((el) => {
...
const navListEl = document.querySelector(".main-section nav ul");
